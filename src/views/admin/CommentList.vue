    ...
    <el-card>
      <el-table :data="comment_list" style="width: 100%;font-size:14px;">
        ...
        <el-table-column label="标题" align="center">
          <template v-slot:default="comment">
            <span
              ...
              v-if="comment.row.content_id"
            >{{ comment.row.content_title }}</span>
        ...
        <el-table-column align="center" label="状态" header-align="center">
          <template v-slot:default="comment">
            <el-tag
              :type="comment.row.status ? '' : 'warning'"
        ...
        <el-table-column align="center" label="操作" header-align="center">
          <template v-slot:default="comment">
            <el-button
              type="text"
              size="small"
              v-if="!comment.row.status"
              @click="handlePass(comment.row.id)"
            >通过</el-button>
...
export default {
  ...
  methods: {
    handlePass(id) {
      ...
      this.$axios
        .get(this.HOST + `/admin/api/passComment?${stringify(params)}`)
        .then(response => {
    ...
    submitDelete() {
      this.loading = true;
      this.$axios
        .delete(this.HOST + `/admin/comment/del/` + this.del_params.id)
        .then(response => {
          if (response.data.code === 0) {
            this.del_visible = false;
            this.handleCurrentPageChange(this.page);
    ...
    handleCurrentPageChange: function(val) {
      ...
      this.$axios
        .get(this.HOST + `/admin/comment/list?${stringify(queryData)}`)
        .then(response => {
          if (response.data.success) {
            let resp = response.data;
            this.total = resp.total;
            (this.comment_list = resp.data), (this.page = val);
          }
        })
        .finally(() => {
          this.loading = false;
